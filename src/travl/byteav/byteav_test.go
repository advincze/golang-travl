...
func TestEmptyByteSet(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			t.Error("A zero-length byteav should be fine")
		}
	}()
	b := New(0)
	if b == nil {
		t.Error("byteav should not be nil")
	}
}
...
func TestEqualEmpty(t *testing.T) {
	...
	if act := New(0); !b.Equal(act) {
		t.Error("empty byteavs should be equal")
	}
}
...
func TestDifferentLegthNotEqual(t *testing.T) {
	...
	if act := New(1); b.Equal(act) {
		t.Error("empty byteavs with different length should not be equal")
	}
}
